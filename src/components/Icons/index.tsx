import { Icon } from "@iconify/react";

export const Icons = {
  playIcon: <Icon icon="solar:play-line-duotone" style={{ marginLeft: 2 }} />,
  pauseIcon: <Icon icon="solar:pause-line-duotone" />,
  hamburguerIcon: <Icon icon="solar:hamburger-menu-broken" />,
  addIcon: <Icon icon="basil:add-outline" />,
  closeIcon: <Icon icon="bx:cross" style={{ rotate: "45deg" }} />,
  timelineIcon: (
    <Icon icon="fluent:timeline-24-regular" style={{ height: 18, width: 18 }} />
  ),
  loadingIcon: <Icon icon="eos-icons:loading" />,
  returnIcon: (
    <Icon icon="akar-icons:arrow-back" style={{ height: 18, width: 18 }} />
  ),
  reloadIcon: (
    <Icon icon="solar:restart-outline" style={{ height: 18, width: 18 }} />
  ),
  triangleIcon: (
    <Icon icon="bi:diamond-half" style={{ height: 12, width: 12 }} />
  ),
  walkArrow: <Icon icon="bi:arrow-bar-right" />,
};
